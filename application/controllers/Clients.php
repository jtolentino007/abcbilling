...
class Clients extends CORE_Controller {
    ...
    public function index()
    {
        ...
        $data['general_docs'] = $this->Documents_model->get_list('is_active=TRUE AND is_deleted=FALSE AND document_category_id=1');
        $data['supporting_docs'] = $this->Documents_model->get_list('is_active=TRUE AND is_deleted=FALSE AND document_category_id=2');
        $data['previous_returns'] = $this->Documents_model->get_list('is_active=TRUE AND is_deleted=FALSE AND document_category_id=3');
        $data['business_styles'] = $this->Business_style_model->get_list('is_active=TRUE AND is_deleted=FALSE');
    ...
    function transaction($txn=null,$filter_value=null){
        switch($txn){
            ...
            //****************************************************************************************************************
            case 'get-client-ind-documents':
                $m_customers=$this->Customers_model;

                $customer_id=$this->input->post('Id');

                $response['data'] = $m_customers->get_list(
                    'customers_info.is_deleted=FALSE AND customers_info.customer_id='.$customer_id,
                    'customer_items.*',
                    array(
                        array('customer_items','customer_items.customer_id=customers_info.customer_id','inner')
                    )
                );
            ...
            case 'create':
                ...
                $previous_id = $this->input->post('previous_id',TRUE);
                $pr_document_type_id = $this->input->post('pr_document_type_id',TRUE);

                for($i=0;$i<count($document_id);$i++)
                {
                    $m_customers_item->customer_id = $customer_id;
                    $m_customers_item->document_id = $document_id[$i];
                    $m_customers_item->document_type = $document_type[$i];
                ...
                {
                    $m_customers_item->customer_id = $customer_id;
                    $m_customers_item->document_id = $previous_id[$i];
                    $m_customers_item->value = $value[$i];
                    $m_customers_item->document_type = $pr_document_type_id[$i];
                    $m_customers_item->save();
                }
            ...
            case 'update':
                ...
                $m_customers->modified_by=$this->session->user_id;
                $m_customers->modify($customer_id);

                $m_customers_item->delete_via_fk($customer_id);
