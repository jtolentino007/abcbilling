...
<html lang="en">
                    ...
                    <div class="static-content"  >
                        <div class="page-content"><!-- #page-content -->
                            ...
                            <ol class="breadcrumb">
                                ...
                                <li><a href="Business_style">Business Style</a></li>
                            </ol>
                                                ...
                                                <h1 style="padding-left: 20px;"><span class="fa fa-building-o" style="border: 3px solid rgb(76, 175, 80); padding: 10px 12px 10px 12px; border-radius: 50%; color: rgb(76, 175, 80);"></span> Business Style <small> | Reference  </small></h1><hr>
                                                    ...
                                                    <div class="panel-body table-responsive" >
                                                        ...
                                                        <table id="tbl_business_style" class="table-striped custom-design" cellspacing="0" width="100%">
                                                            <thead>
                                                                <tr>
                                                                    <th>Business Style Name</th>
                                                                    <th>Business Style Category</th>
                                                                    <th>
    ...
    $(document).ready(function(){
        ...
        var initializeControls=function(){
            dt=$('#tbl_business_style').DataTable({
                "dom": '<"toolbar">frtip',
                "bLengthChange":false,
                "language": {
                    "searchPlaceholder":"Search Business Style"
                },
                "ajax" : "Business_style/transaction/list",
                "columns": [

                    { targets:[0],data: "business_style_name" },
                    { targets:[1],data: "business_style_description" },
                    {
                        targets:[2],
                        render: function (data, type, full, meta){
                            var btn_edit='<button class="btn btn-primary btn-sm <?php echo (in_array('1-6-e',$this->session->user_rights)?'':'hidden'); ?>" name="edit_info"  style="margin-left:-15px;" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil"></i> </button>';
                            var btn_trash='<button class="btn btn-danger btn-sm <?php echo (in_array('1-6-d',$this->session->user_rights)?'':'hidden'); ?>" name="remove_info" style="margin-right:0px;" data-toggle="tooltip" data-placement="top" title="Move to trash"><i class="fa fa-trash-o"></i> </button>';
            ...
            var createToolBarButton=function(){
                var _btnNew='<button class="btn btn-primary <?php echo (in_array('1-6-a',$this->session->user_rights)?'':'hidden'); ?>"  id="btn_new" style="text-transform: capitalize;font-family: Tahoma, Georgia, Serif;" data-toggle="modal" data-target="" data-placement="left" title="New Business Style" >'+
                    '<i class="fa fa-plus-circle"></i> New Business Style</button>';
        ...
        var bindEventHandlers=(function(){
            ...
            $('#tbl_business_style tbody').on('click','button[name="edit_info"]',function(){
                _txnMode="edit";
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID = data.business_style_id;
            ...
            $('#tbl_business_style tbody').on('click','button[name="remove_info"]',function(){
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID = data.business_style_id;
            ...
            $('#btn_yes').click(function(){
                removeBusinessStyle().done(function(response){
            ...
            $('#btn_save').click(function(){
                if(validateRequiredFields()){
                    if(_txnMode=="new"){
                            ...
                            updateBusinessStyle().done(function(response){
                            showNotification(response);
        ...
        var validateRequiredFields=function(){
            ...
            $('input[required],textarea[required]','#frm_BusinessStyle').each(function(){
                if($(this).val()==""){
        ...
        var updateBusinessStyle=function(){
            var _data=$('#frm_BusinessStyle').serializeArray();
            _data.push({name : "business_style_id" ,value : _selectedID});

            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Business_style/transaction/update",
                "data":_data,
                "beforeSend": showSpinningProgress($('#btn_save'))
            });
        };

        var removeBusinessStyle=function(){
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Business_style/transaction/delete",
                "data":{business_style_id : _selectedID}
            });
        };

        var showList=function(b){
            if(b){
                $('#div_BusinessStyle_list').show();
                $('#div_category_fields').hide();
            }else{
                $('#div_BusinessStyle_list').hide();
        ...
        var clearFields=function(){
            $('input[required],textarea','#frm_BusinessStyle').val('');
